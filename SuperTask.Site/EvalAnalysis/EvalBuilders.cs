...
namespace TheSite.EvalAnalysis
{
	...
	public class EvalIndicationBuilder : IEvalIndicationBuilder
	{
		...
		{
			return evalIndications.Where(x => x.FullScore > 0).Sum(e => e.FullScore);
		}
	}
	...
	{
		public static Dictionary<Guid, IEvalIndicationBuilder> Builders = new Dictionary<Guid, IEvalIndicationBuilder>
	  {
		 {IndicationKeys.SubjectType,new SubjectEvalBuilder() },
		 {IndicationKeys.AutoType,new AutoEvalBuilder() },
		 {EvalTableKeys.SubjectType,new SubjectEvalBuilder() },
		 {EvalTableKeys.AutoType,new AutoEvalBuilder() },
	  };
	}
