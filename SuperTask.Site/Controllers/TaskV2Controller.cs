	...
	public class TaskV2Controller : BaseController
	{
		...
		public ActionResult Start(OperationViewModel model)
		{
			...
			try
			{
				...
				{
					db.OperationDal.Insert(new Operation(model.ProjectId, id, TaskKeys.StartActionGuid, model.Result, model.Result2, DateTime.Now, assignId, model.Remark));
					APQuery.update(t)
		...
		public ActionResult Close(OperationViewModel model)
		{
			...
			try
			{
				foreach (var item in tasks)
				{
					var id = item.TaskId;
					var workhours = item.WorkHours;
					db.OperationDal.Insert(new Operation(model.ProjectId, id, TaskKeys.CloseActionGuid, model.Result, model.Result2, DateTime.Now, assignId, model.Remark));
					APQuery.update(t)
