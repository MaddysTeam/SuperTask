					  ...
					  int current, int rowCount, AjaxOrder sort, string searchPhrase)
		{
			...
			if (sort != null)
			{
				switch (sort.ID)
				{
					...
					case "Level": results = sort.According == APSqlOrderAccording.Asc ? results.OrderBy(x => x.BugLevel).ToList() : results.OrderByDescending(x => x.BugLevel).ToList(); break;
					case "BugName": results = sort.According == APSqlOrderAccording.Asc ? results.OrderBy(x => x.BugName).ToList() : results.OrderByDescending(x => x.BugName).ToList(); break;
					case "Type": results = sort.According == APSqlOrderAccording.Asc ? results.OrderBy(x => x.BugType).ToList() : results.OrderByDescending(x => x.BugType).ToList(); break;
					case "Status": results = sort.According == APSqlOrderAccording.Asc ? results.OrderBy(x => x.BugStatus).ToList() : results.OrderByDescending(x => x.BugStatus).ToList(); break;
					case "FixDateStr": results = sort.According == APSqlOrderAccording.Asc ? results.OrderBy(x => x.FixDate).ToList() : results.OrderByDescending(x => x.FixDate).ToList(); break;
		...
		public ActionResult Edit(Bug bug, FormCollection collection)
		{
			...
			try
			{
				...
				if (bug.BugId.IsEmptyGuid())
				{
					...
					// operation record
					db.OperationDal.Insert(new Operation(bug.Projectid, bug.BugId, BugKeys.CreateGuid, BugKeys.CreateGuid.ToString(), null, DateTime.Now, user.UserId, string.Empty));
				}
				else
				{
					...
					// operation record
					db.OperationDal.Insert(new Operation(bug.Projectid, bug.BugId, BugKeys.EditGuid, BugKeys.EditGuid.ToString(), null, DateTime.Now, user.UserId, string.Empty));
				}
