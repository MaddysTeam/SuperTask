...
namespace TheSite.Controllers
{
	...
	public class ShareFolderController : BaseController
	{
		...
		public ActionResult Search(Guid parentId, string projectId, string phrase)
		{
			...
			var allFiles = GetFiles(phrase);
			var parents = allFolders.FindAll(f => f.id == parentId);
			// child files
			var files= new List<FileViewModel>();
			files.AddRange(allFiles.FindAll(x => x.folderId == parentId));
			...
			var children = FindChildFolders(parentId, GetAllFolders(), new List<FolderViewModel>(), allFiles, neglectId);
			if (children != null && children.Count > 0)
			{
				folders.AddRange(parents.Concat(children));
				// get all files from child folders
				files.AddRange(children.SelectMany(x => x.files)?.ToList());
			}
		...
		public ActionResult FileEdit(FileViewModel file)
		{
			if (file.id.IsEmpty())
			{
				if (GetFiles(file.name, new Guid[] { file.folderId }).Count > 0)
				{
					return Json(new
					{
		...
		public ActionResult SendFileToFolder(Guid folderFileId, Guid originFolderId, Guid targetFolderId)
		{
			...
			if (attachment != null && GetFiles(attachment.RealName, new Guid[] { targetFolderId }).Count > 0)
			{
				return Json(new
				{
					result = AjaxResults.Error,
			...
			for (var i = 0; i < folders.Count; i++)
			{
				...
				if (folder.parentId == parentId && parentId != neglectId)
				{
					if (!result.Exists(x => x.id == folder.id))
					{
						...
						folder.files.AddRange(allFiles.FindAll(x => x.folderId == folder.id));
					}
		...
		private List<FileViewModel> GetFiles(string phrase = null, Guid[] folderIds = null)
		{
			...
			{
				query = query.where(ff.FolderId.In(folderIds));
			}
