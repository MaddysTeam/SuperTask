...
namespace TheSite.Controllers
{
   ...
   public class ShareFolderController : BaseController
   {
      ...
      // POST: ShareFolder/SetPermission 
      public ActionResult SetPermission(Guid folderId, Guid fileId)
      {
         ...
         if (fileId.IsEmpty())
            models = db.FolderPermissionDal.ConditionQuery(fp.FolderId == folderId, null, null, null);
         else
            models = db.FolderPermissionDal.ConditionQuery(fp.FolderFileId == folderId, null, null, null);

         if (models.Count > 0)
         {
            viewModel.FileId = models[0].FolderFileId;
            viewModel.FolderId = models[0].FolderId;
            viewModel.PermissionIds = string.Join(",", models.Select(x => x.PermissionId));
            viewModel.UserIds = string.Join(",", models.Select(x => x.PermissionId));
         }
      ...
      public ActionResult SetPermission(FolderPemissionViewModel model)
      {
         ...
         if (model.FileId == null) // set permission for folder only
         {
         ...
         {
            db.FolderPermissionDal.ConditionDelete(fp.FolderFileId == model.FileId);
