...
namespace TheSite.Controllers
{
   ...
   public class ShareFolderController : BaseController
   {
      ...
      public ActionResult Search(Guid parentId, string phrase)
      {
         ...
         //if (!HasFolderPermission(userId, parentId, ShareFolderKeys.SearchPermission))
         //{
         //   return Json(new
         //   {
         ...
         var children = FindChildFolders(parentId, GetAllFolders(), new List<FolderViewModel>(), files);
      ...
      public ActionResult SetFolderPermission(FolderPemissionViewModel model)
      {
         ...
         var files = db.FolderFileDal.ConditionQuery(fp.FolderId.In(folderIds), null, null, null);
         ...
         if (files.Count > 0)
         {
            foreach (var item in files)
               foreach (var userId in model.UserGuids)
                  foreach (var permissionId in model.PermissionGuids)
                     db.FolderPermissionDal.Insert(new FolderPermission(item.FolderFileId, model.FileId, userId, permissionId));
         }
      ...
      private List<FolderViewModel> FindChildFolders(Guid parentId, List<FolderViewModel> folders, List<FolderViewModel> result, List<FileViewModel> allFiles)
      {
         ...
         for (var i = 0; i < folders.Count; i++)
         {
            ...
            if (folder.parentId == parentId)
            {
               ...
               FindChildFolders(folder.id, folders, result, allFiles);
            }
         }
