...
namespace Business
{
   ...
   public class WorkJournalEditHandler : IHandler<WorkJournal, WorkJournalEditOption>
   {
      ...
      public virtual void Handle(WorkJournal journal, WorkJournalEditOption option)
      {
         ...
         else
         {
            ...
            try
            {
               ...
               UpdateTaskHours(journal, option);
      ...
      /// </summary>
      protected virtual void UpdateTaskHours(WorkJournal journal, WorkJournalEditOption option)
      {
         var db = option.db;
        ...
        // var parentJournals = new List<WorkJournal>();
         ...
         if (tk != null)
         {
            ...
            //累计当前任务实际工时（累加）
            db.WorkTaskDal.UpdatePartial(tk.TaskId, new
            {
               WorkHours = GetCurrentValue(tk.WorkHours, original.WorkHours, journal.WorkHours),
